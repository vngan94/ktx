...
const ManageRegulation = () => {
  ...
  let handleOnClickExport = async () => {
    ...
    if (res && res.errCode == 0) {
      await CommonUtils.exportExcel(res.data, "Danh sách giường", "ListBed");
    }
  };
  let handleBlockBed = async (event, id, code, status) => {
    ...
    if (status == 1) {
      ...
      if (res && res.errCode == 0) {
        toast.success("Khóa giường thành công");
        let res = await getAllCodeByTypeStatus({
          limit: PAGINATION.pagerow,
        ...
        toast.error("Khóa giường thất bại");
      }
    } else {
      ...
      if (res && res.errCode === 0) {
        toast.success("Mở khóa giường thành công");
        let res = await getAllCodeByTypeStatus({
          limit: PAGINATION.pagerow,
          offset: numberPage * PAGINATION.pagerow,
          keyword: keyword,
        ...
        console.log(res.errCode);
        toast.error("Mở giường thất bại");
      }
    }
  };
  let handleDeleteBed = async (event, id) => {
    event.preventDefault();
    ...
    if (res && res.errCode === 0) {
      ...
      console.log(res.errCode);
      toast.error("Không cho phép xóa giường này");
    }
  };
                  ...
                  dataBed.map((item, index) => {
                    return (
                      <tr key={index}>
                        ...
                        <td>
                          <Link to={`/admin/edit-bed/${item.id}`}>
                            Chỉnh sửa
                          </Link>
                          &nbsp; &nbsp;
                          <a
                          ...
                          <a
                            href="#"
                            onClick={(event) => handleDeleteBed(event, item.id)}
                          >
                            Xóa
                          </a>
                        </td>
